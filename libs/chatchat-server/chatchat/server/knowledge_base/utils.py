...
#loader不同文件匹配不同的加载器-加载器将不同的文件，都转换成LangchainDounment对象--纯粹的字符文本集
LOADER_DICT = {
...
#---让系统只加载一次tokenizer，这样以后每一个用户上传一个文件都调用这一个tokenizer--内存缓存机制
#--上千行的组装代码在整个生命周期真的只会被运行一次！之后再有 100 个人同时切文件，全部从系统内存的高速缓存里直接复用同一个对象。
@lru_cache()
    ...
    try:
        if (
            ...
            try:  # 优先使用用户自定义的text_splitter
                ...
                TextSplitter = getattr(text_splitter_module, splitter_name)
            #按照什么方式切分文本，
            # 1.openai -tiktoken-按token收费 
            # 2.huggingface --AutoTokenizer（分词器），用这套模型“土生土长”的算数方法来切分文本。 
            # 3.spacy-管它是怎么变 Token 的了，我直接就按照字符数量或者中文的词汇来硬切
            if (
...
class KnowledgeFile:
    ...
    #通过判断文件后缀，获取相应格式的文件加载器，然后把文件加载为Langchain的Document对象{page_context,metadata}
    def file2docs(self, refresh: bool = False):
        if self.docs is None or refresh:
            ...
            if isinstance(loader, TextLoader):
                ...
                self.docs = loader.load()
        return self.docs
    #对Document对象进行切分为chunk，但是也是用Document对象表示。
    def docs2texts(
        self,
