...
def upload_docs(
    ...
    # 2.对保存的文件进行向量化-- 根据to_vector_store决定是否向量化
    if to_vector_store:
        result = update_docs(
...
def update_docs(
    ...
    # 多线程迭代器files2docs_in_thread --进行chunk
    for status, result in files2docs_in_thread(
        ...
        if status:
            kb_name, file_name, new_docs = result ## 拿到了切好的几十个或者几百个文本块
            kb_file = KnowledgeFile(
                filename=file_name, knowledge_base_name=knowledge_base_name
            )#定义k文件实体
            #这是切分的文本块
            kb_file.splited_docs = new_docs ## 把文本块塞到对象属性里
            #更新向量库
    ...
    for file_name, v in docs.items():
        try:
            ...
            # 💥 把这些文本块统统送进 Embedding 模型变向量，并且插到向量数据库里 -这里变成向量
