...
def create_models_chains(
    ...
    """
    为大模型赋予记忆History、手脚Tools以及执行逻辑AgentExecutor。
    """
    #1.在数据库中取记忆-并排序
    # 查数据库--从数据库获取conversation_id对应的 intermediate_steps 、 mcp_connections --获取对话历史上下文并排序
    messages = filter_message(
        conversation_id=conversation_id, limit=history_len
    )
    # 返回的记录按时间倒序，转为正序--并连接历史上下文，把上次的回答和工具调用记录连接起来。
    ...
    #3.装配执行逻辑AgentExecutor--通过 PlatformToolsRunnable.create_agent_executor 把模型、历史、工具集全部揉成一个 Langchain 可执行的 agent_executor 链（Chain）。
    #LLM+ 工具+ 对话历史 = Agent
    agent_executor = PlatformToolsRunnable.create_agent_executor(
        ...
        intermediate_steps=intermediate_steps, # 中间步骤
        mcp_connections=mcp_connections if use_mcp else {} # MCP连接
    )
    #agent执行流程 -- 传入用户输入 -- Agent -工具调用 - LL回答 
...
async def chat(
    ...
    """Agent 对话
    作为对外的接口,调度create_models_from_config和create_models_chains,接收用户语句,并以类似 OpenAI 的流式格式输出
    """
    #1.异步迭代器
    async def chat_iterator_event() -> AsyncIterable[OpenAIChatOutput]:
        try:
            ...
            #一上来先尝试挂载 Langfuse 密钥，因为复杂的 Agent 对话如果出错了很难排查，加入 Langfuse 回调可以进行可视化链路追踪。
            import os
            ...
            #调用create_models_from_config创建模型
            ...
            #调用create_models_chains创建AgentExecutor
            ...
            #将用户输入添加到数据库 -根据conversation_id查询
            message_id = add_message_to_db(
                    chat_type="llm_chat",
                    query=query,
                    conversation_id=conversation_id, 
            )
            #调用AgentExecutor执行对话 --用户问题 ->Agent执行
            chat_iterator = full_chain.invoke({
                "input": query
            })
            last_tool = {}
            #agent 执行，由agent返回的item -  agent可能做的事（LLM输出、调用工具、工具执行结果、最终回答）
            #一直循环，直到agent执行完成，每次循环agent做的事情可能不一样所以通过isinstance判断item类型
            async for item in chat_iterator:
                ...
                #判断item类型-通过大量的 isinstance 判断当前 Agent 抛出来的 item 是属于哪一类：
                #PlatformToolsAction：模型说我准备调用工具啦。
                #PlatformToolsActionToolStart / End：工具开始执行/执行完毕。
                #PlatformToolsFinish：工具执行完了，把结果还给模型。
                #PlatformToolsError：工具调用出错了。
                #PlatformToolsMessage：模型直接输出了一段文字（非工具调用）。
                #PlatformToolsFinalAnswer：模型觉得已经有最终答案了，不再需要工具了。
                ...
                #封装成了兼容 OpenAI 接口规范的 OpenAIChatOutput，使用 Yield 配合 EventSourceResponse，实现了前端那种“大模型打字机”并且能“实时展示工具调用进度”的UI效果。
